...
# ESCピン定義
esc_Pins = [17, 6]  # ピン指定
esc_Devices = [PWMOutputDevice(pin, frequency=1000) for pin in esc_Pins]  # PWMデバイスの作成
esc_Pulsewidth = [MIN_SIGNAL, MIN_SIGNAL]  # ESCの初期パルス幅

# パルス幅を0~1の範囲に変換する関数
def pulsewidth_to_duty_cycle(pulsewidth):
    return (pulsewidth - MIN_SIGNAL) / 10000
